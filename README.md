...
- **실제 서비스에 가까운 데이터 흐름**  
  - SQLite 기반 로컬 DB + 화면별 CRUD 흐름이 명확합니다.

- **멀티 스크린 & 네비게이션 설계**  
  - 메인/등록/수정/상세/통계/캘린더/촬영 등 다양한 화면을 Stack/Tab 네비게이션으로 구성했습니다.

- **시각화 + 카메라 기능 통합**  
  - 단순 리스트를 넘어 **차트, 캘린더, 카메라, 갤러리**를 모두 사용하는 프로젝트입니다.

- **타입스크립트 기반 구조화**  
  - `data`, `hooks`, `utils`, `components`, `screens`, `navigations`로 모듈을 분리하여 유지보수성과 확장성을 고려했습니다.

향후에는 **스크린샷 섹션**과 **개발 회고 링크**를 추가하면 GitHub 포트폴리오용으로 더 보기 좋은 README가 될 수 있습니다.
